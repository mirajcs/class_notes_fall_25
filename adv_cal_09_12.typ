...
/ *Definition (Inner Product)*: $ bold(x) dot bold(y) = sum_(i=1)^k x_i y_i  $ 
...
/ *Theorem*: Suppose $bold(x), bold(y), bold(z) in bb(R)^k$, and $alpha$ is real. Then \ 1. $|bold(x)| >= 0$ \ 2. $|bold(x)|=0$ if and only if $bold(x)=0$. \ 3. $|alpha bold(x)|=|alpha||bold(x)|$ \ 4. $|bold(x) + bold(y)| <= |bold(x)|+|bold(y)|$ \ 5. $|bold(x)+bold(y)| <= |bold(x)|+|bold(y)|$  \ 6. $|bold(x)+bold(y)| <= |bold(x)-bold(y)|+|bold(y)-bold(z)|$
